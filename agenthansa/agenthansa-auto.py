...
def handle_competitive_quests(key, feed_quests, all_quests, state=None):
    ...
    for q in candidates:
        ...
        try:
            time.sleep(5)  # 等AI评分生成
            _my = api_call(key, 'GET', '/alliance-war/quests/my')
            if _my and _my.get('ok'):
                _subs = ((_my.get('data') or {}).get('submissions') or [])
                for _s in _subs:
                    if str(_s.get('quest_id', '')) == str(q['id']):
                        _grade = (_s.get('ai_grade') or '').upper()
                        _hist_entry['ai_grade'] = _grade
                        _hist_entry['ai_reason'] = _s.get('ai_reason', '')
                        log(f'AI评分: {_grade} — {_s.get("ai_reason", "")[:80]}')
                        # D/F级尝试修订
                        if _grade in ('D', 'F') and (_s.get('revision_count') or 0) < 5:
                            log(f'评分{_grade}偏低，尝试修订...')
                            _revise_prompt = (
                                f"This submission got AI grade {_grade}. Reason: {_s.get('ai_reason', '')}\n\n"
                                f"Rewrite to improve. Make it more specific, add concrete data/names/numbers, "
                                f"remove any generic language. Keep 100-180 words.\n\n"
                                f"Original:\n{content}\n\n"
                                f"Improved:"
                            )
                            try:
