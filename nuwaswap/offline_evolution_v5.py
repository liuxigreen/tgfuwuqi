...
# ---------------------------------------------------------
# 1. 极速读取本地语料库 (合并 V1, V2, V3, V4 共 400 人)
# ---------------------------------------------------------
...
def load_offline_corpus():
    corpus = []
    for f in [DB_FILE_V1, DB_FILE_V2, DB_FILE_V3, DB_FILE_V4]:
        if os.path.exists(f):
            with open(f, "r", encoding="utf-8") as file:
                corpus.extend(json.load(file))
            
    print(f"📚 成功加载【V5 终极合并语料库】！共计 {len(corpus)} 条顶级机构与交易员策略（包含 400 名交易员基因）")
    return corpus
...
# ---------------------------------------------------------
# 4. V5 大逃杀引擎 (5大时间级别全面碰撞)
# ---------------------------------------------------------
class OfflineEvolutionEngineV5:
    def __init__(self, dfs, corpus):
        self.dfs = dfs
        self.corpus = corpus

        # 初始基因，涵盖从高频到宏观的五个时间周期
        self.dna = {
            "5m":  {"ema_f": 40, "ema_s": 100, "rsi_os": 30, "rsi_ob": 70, "sl": 0.0500, "tp": 0.1500}, # 尝试用极宽的止损救活5m
            "15m": {"ema_f": 40, "ema_s": 100, "rsi_os": 30, "rsi_ob": 70, "sl": 0.0500, "tp": 0.1500},
            ...
            "4h":  {"ema_f": 48, "ema_s": 100, "rsi_os": 35, "rsi_ob": 65, "sl": 0.0896, "tp": 0.3162}
        }
    ...
    def distill_from_corpus(self):
        """每轮从 400 人机构语料库中随机抽取 12 条进行多维解读，引发参数漂移"""
        sample = random.sample(self.corpus, min(12, len(self.corpus)))
        mutation = {"macro_institution": 0, "micro_scalper": 0}

        for item in sample:
            txt = (item.get("snippet", "") + item.get("title", "")).lower()
            if "arbitrage" in txt or "market making" in txt or "macro" in txt or "trend" in txt or "liquidity" in txt: 
                mutation["macro_institution"] += 1
            if "scalp" in txt or "fast" in txt or "stop" in txt or "short" in txt or "day" in txt: 
                mutation["micro_scalper"] += 1
        ...
        for tf in self.dna.keys():
            ...
            if mutation["micro_scalper"] >= mutation["macro_institution"]:
                p["sl"] = max(0.003, p["sl"] - 0.002 + noise_sl)
                p["tp"] = max(0.010, p["tp"] - 0.005 + noise_tp)
                p["ema_f"] = max(5, p["ema_f"] - 2)
            else:
                p["sl"] = min(0.15, p["sl"] + 0.002 + noise_sl)
                p["tp"] = min(0.40, p["tp"] + 0.005 + noise_tp)
                p["ema_f"] = min(120, p["ema_f"] + 2)
    ...
    def eval_timeframe(self, tf):
        ...
        elif random.random() > 0.4: # 60%概率回退到历史最优
            self.dna[tf] = self.best_dna[tf].copy()

        return cap, trades, win_rate, pnl

    def run_400_rounds(self):
        print(f"\n⚔️ 【V5 全周期大搏杀启动】包含 5m, 15m, 30m, 1h, 4h | 400名机构与大V基因深度碰撞")
        ...
        print("\n🏆 400 轮宏观+高频进化结束！400名机构/交易员智慧结晶的【终极宇宙基因】(The Universal Apex DNA)：")
        for tf in self.dna.keys():
...
if __name__ == "__main__":
    ...
    if corpus:
        ...
        engine = OfflineEvolutionEngineV5(dfs, corpus)
        engine.run_400_rounds()
