...
# ---------------------------------------------------------
# 1. 极速读取本地语料库 (Zero API Latency, 合并 V1, V2 和 V3)
# ---------------------------------------------------------
...
def load_offline_corpus():
    ...
    print(f"📚 成功加载终极合并语料库！共计 {len(corpus)} 条顶级交易员策略（包含 V1, V2, V3 共 300 名交易员）")
    return corpus
...
# ---------------------------------------------------------
# 4. 300 轮达尔文大逃杀引擎 (The 300-Round Crucible - 300 Traders)
# ---------------------------------------------------------
class OfflineEvolutionEngineV3:
    def __init__(self, dfs, corpus):
        ...
        # 从 V2 的巅峰基因开始起步
        self.dna = {
            "5m":  {"ema_f": 48, "ema_s": 100, "rsi_os": 30, "rsi_ob": 70, "sl": 0.0447, "tp": 0.0951},
            "15m": {"ema_f": 50, "ema_s": 100, "rsi_os": 30, "rsi_ob": 70, "sl": 0.0429, "tp": 0.1098},
            "30m": {"ema_f": 21, "ema_s": 50, "rsi_os": 30, "rsi_ob": 70, "sl": 0.0167, "tp": 0.0460},
            "1h":  {"ema_f": 50, "ema_s": 200, "rsi_os": 30, "rsi_ob": 70, "sl": 0.0500, "tp": 0.1500}
        }
        self.best_pnl = {"5m": -999, "15m": -999, "30m": -999, "1h": -999}
        self.best_dna = {k: v.copy() for k, v in self.dna.items()}

    def distill_from_corpus(self):
        """每轮从合并的语料库中随机抽取 8 条进行解读，引发参数漂移"""
        sample = random.sample(self.corpus, min(8, len(self.corpus)))
        mutation = {"tight": 0, "wide": 0}

        for item in sample:
            txt = (item.get("snippet", "") + item.get("title", "")).lower()
            if "tight" in txt or "scalp" in txt or "fast" in txt or "stop" in txt or "short" in txt or "setup" in txt: mutation["tight"] += 1
        ...
        for tf in self.dna.keys():
            ...
            if mutation["tight"] >= mutation["wide"]:
                ...
                p["sl"] = min(0.06, p["sl"] + 0.001 + noise_sl)
                p["tp"] = min(0.18, p["tp"] + 0.002 + noise_tp)
                p["ema_f"] = min(60, p["ema_f"] + 1)
    ...
    def run_300_rounds(self):
        print(f"\n⚔️ 【V3 离线极速进化启动】0 API 延迟 | 4 大时间级别 | 300名顶级交易员基因碰撞")
        ...
        print("\n🏆 300 轮进化结束！300名交易员智慧结晶的最强基因 (The Apex DNA V3)：")
        for tf in self.dna.keys():
...
if __name__ == "__main__":
    ...
    if corpus:
        ...
        engine = OfflineEvolutionEngineV3(dfs, corpus)
