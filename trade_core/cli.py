...
def main() -> None:
    ...
    if args.cmd == "validate-config":
        print(json.dumps(validate_config(config), ensure_ascii=False)); return
    ...
    if args.cmd == "replay":
        if args.journal:
            print(json.dumps(replay_journal(args.journal), ensure_ascii=False)); return
    ...
    if args.cmd == "pipeline":
        print(json.dumps(run_pretrade_pipeline(_read_json(args.sample), mode=args.mode), ensure_ascii=False)); return

    if args.cmd == "execute":
        print(json.dumps(run_demo_execution_pipeline(_read_json(args.sample), demo_execute=args.demo_execute, backend=args.backend, profile=args.profile), ensure_ascii=False)); return

    if args.cmd == "manage-position":
        print(json.dumps(run_position_management(_read_json(args.sample)), ensure_ascii=False)); return

    if args.cmd == "review":
        print(json.dumps(review_journal(args.journal), ensure_ascii=False)); return

    if args.cmd == "report":
        rep = build_daily_report(args.journal)
        print(json.dumps(model_to_dict(rep), ensure_ascii=False)); return
