...
def base_inputs():
    ...
    return radar, nuwa, market, senti, smart


def test_resonance_score_higher():
    radar, nuwa, market, senti, smart = base_inputs()
...
def test_premium_too_high_penalized():
    radar, nuwa, market, senti, smart = base_inputs()
    normal = compute_score(radar, nuwa, market, senti, smart).total_score
    smart.premium_discount_pct = 1.5
    penalized = compute_score(radar, nuwa, market, senti, smart)
    assert penalized.total_score < normal
    assert "smartmoney_premium_too_high" in penalized.reason_codes
